...
```{r, results='asis'}
dimnames(trialnr) <- list(rownames(trialnr, do.NULL = FALSE, prefix = "pp "),
...
for (i in 1:length(dflist)) {
  ...
  order8 <- dflist[[i]]$Prob1 # lookup order from orders4.txt, which is unnecessary
  row = c()
  for(j in 1:nrow(df4)) {
    row[j] <- all(as.numeric(df4[j,] ) == order8) # check which row has the same order
  }
  row.names(df4[row,])
...
name = paste("../figs/knit/ev-finaldata-boxplots", 
