...
getParams <- function(data, bbanditsfunc) {
	...
	fits <- matrix(data = 0, nrow = length(unique(subjects)), ncol = 6)
	fits[,6] = subjects
	...
	for (id in 1:length(unique(subjects))) {  
		...
		if (MAXITERBB > 0 ) {
			fits[id,1] <- median(apply(bb.lik,1,min), na.rm = T)
			fits[id,2] <- median(apply(bb.tht,1,min), na.rm = T)
			fits[id,3] <- median(apply(bb.tht,1,min), na.rm = T)
		}
	}
	
	fits[,4] <- 2 * fits[,1] + 2 * log(100) # BIC: deviance + #parameters * log(N)
	fits[,5] <- 2 * fits[,1] + 2 * 2        # AIC: deviance + #parameters * 2
	fits
}
...
getPredictions <- function(fits) {
	...
	preds <- array(data = NA, dim = c(length(subjects)*8, 100, 2*N_BANDITS)) 
	for (id in 1:length(unique(subjects))){
		thetaBB <- fits[id,2];
		subdata = data[data$id == unique(subjects)[id],]
		for (games in 1:length(unique(data$block_id[data$id == unique(subjects)[id]]))) {
			...
			for (trial in c(0:99)) {
				...
				preds[(id - 1) * 8 + games, trial + 1, 1:N_BANDITS] <- orderedprobs
				preds[(id - 1) * 8 + games, trial + 1, N_BANDITS + ind] <- 1
				wins     [choice] <- wins     [choice] + R
				bantrials[choice] <- bantrials[choice] + 1
			}
		}
	}  
	# print(colMeans(preds, dims = 1, na.rm = T))
}
